...
return new class extends Migration
{
    ...
    {
        $club = Club::where('slug', 'lodge-of-fraternity')->orWhere('id', 1)->first();
        if (! $club) {
            return;
        }

        if ($club->slug !== 'lodge-of-fraternity') {
            $club->update(['slug' => 'lodge-of-fraternity', 'name' => 'The Lodge of Fraternity']);
        }
        ...
        // Also ensure any Club User without a Member record gets linked/created
        $clubUsers = $club->users;
        foreach ($clubUsers as $u) {
