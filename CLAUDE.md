...
- **Member names:** a member's title is never entered. It is their `masonic_rank` (`Member::MASONIC_RANKS`: Bro, WBro, VWBro, RWBro, MWBro) and `Member::title` reads it, so the two cannot disagree; the old `members.title` column is ignored. Show a member with `formatted_rank_name` (rank, name, then grand and provincial rank) and use `full_name` only where no title is wanted. Do not build names by hand from `first_name`/`last_name`, and take the rank dropdown options from `Member::MASONIC_RANKS`.
...
- **Email and newsletters:** the platform's mailables are `ShouldQueue` on the `transactional` queue (so tests use `Mail::assertQueued`), and a worker must run `--queue=transactional,default,bulk`. Newsletters send only through `NewsletterSender` (draft to sent once; `SendNewsletterJob` builds recipients with `NewsletterAudience`, then one `SendNewsletterDelivery` per person on the `bulk` queue, recorded in `newsletter_deliveries`, unique per newsletter and email). Members are signed up to their club's channels unless they opted out (mandatory channels cannot be opted out); visitors need an active subscription. Every email has a private one-click unsubscribe link (`/webhooks/newsletters/unsubscribe/{token}`) and a browser copy (`/site/newsletter/{token}`). The digest (`app:send-weekly-digest --scheduled`, hourly) goes through the same path.
- **Candidate pipeline** (`app/Domains/ClubAccounting/{Enums/CandidateStage,Models/Candidate,Models/CandidateEvent,Services/CandidateTransitionService}.php`, Livewire pages `Candidates/{CandidatePipeline,CandidateDetail}` sharing `Concerns/HandlesCandidateActions`): enquiry → first interview → informal meeting → lodge committee → proposed (Form P, committee recommendation, proposal date) → accepted (ballot result; only elected/not-elected is stored, never vote counts) → initiated (creates a `Member`, unchanged). Every stage change goes through `CandidateTransitionService::transitionStage()` (one step at a time; skipping or going back needs a reason) or `reject()`/`close()`/`hold()`/`reopen()` (owner/admin only), each writing an unchangeable row to `club_acc_candidate_events`; only plain notes/calls/meetings can be edited or removed. The board only shows the six active stages; Initiated/Rejected/Closed live in a "Completed and closed" strip. This is a first pass at UGLE Form P and Rule 159/163 (proposal at a regular meeting, ballot at the next, initiate within a year) built from public secretary handbooks, not verified against the Book of Constitutions — check with a Provincial Secretary before relying on the exact rule numbers or deadlines.
 sign-up confirmation is only enforced once a real mail driver is set (`log`/`array` skip it); `REQUIRE_EMAIL_VERIFICATION` overrides.
