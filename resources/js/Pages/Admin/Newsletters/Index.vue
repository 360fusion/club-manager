...
const sendBroadcast = (id) => {
  if (confirm('Send this newsletter broadcast to all targeted member roles now?')) {
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200/80">
        <div>
          ...
          <p class="text-xs text-slate-500 mt-1">Compose and send email broadcasts targeted to specific member roles.</p>
        </div>
        <Link :href="route('admin.newsletters.create', { clubSlug: club.slug })" class="px-4 py-2.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold rounded-xl shadow-md shadow-amber-600/20 transition-all text-center">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-200/80 space-y-1">
          <div class="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Total Active Subscribers</div>
          <div class="text-2xl font-black text-emerald-600">
            {{ club.members_count || 12 }}
          </div>
        </div>
      </div>
      ...
      <div v-if="newsletters.length" class="grid grid-cols-1 gap-4">
        <div v-for="item in newsletters" :key="item.id" class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/80 hover:border-slate-300 transition-all flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div class="space-y-3">
            ...
            <div class="text-[11px] text-slate-400 font-medium">Estimated Audience: <strong>{{ item.recipient_count }} members</strong></div>
          </div>
      ...
      <div v-else class="bg-white rounded-2xl p-12 text-center shadow-sm border border-slate-200/80 space-y-4">
        ...
        <Link :href="route('admin.newsletters.create', { clubSlug: club.slug })" class="inline-block px-5 py-2.5 bg-amber-600 text-white font-bold text-xs rounded-xl shadow-md shadow-amber-600/20">
