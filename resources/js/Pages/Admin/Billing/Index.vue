  ...
  <AdminLayout title="Main Business Account & Billing Setup" :club="club" active-tab="settings">
    <div class="max-w-5xl mx-auto space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200/80">
        <div>
          <div class="flex items-center gap-2">
            <span class="px-2.5 py-1 bg-slate-900 text-white font-extrabold text-[10px] rounded-full uppercase tracking-wider">Business Administration</span>
            <span class="text-xs font-bold text-slate-500">• {{ club.name }}</span>
          </div>
          <h2 class="text-xl font-bold text-slate-900 mt-1">Main Business Account & Merchant Gateway Administration</h2>
          <p class="text-xs text-slate-500 mt-0.5">Configure your lodge/club legal corporate business entity, merchant payment gateway API credentials for collecting member dues, and manage SaaS platform subscription billing.</p>
        </div>

        <div class="flex items-center gap-2">
          <a
            :href="route('billing.portal', { clubSlug: club.slug })"
            class="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl shadow-md shadow-indigo-600/20 transition-all flex items-center gap-2 cursor-pointer"
          >
            <span>🚀</span>
      ...
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Official company, lodge, or charity business details used for invoicing, tax receipts, and merchant registration.</p>
          </div>
          <span class="text-[11px] font-bold text-slate-500 bg-slate-200/80 px-2.5 py-1 rounded-lg">Corporate Entity</span>
        </div>
        ...
        <form @submit.prevent="saveBusinessAccount" class="p-6 space-y-5 text-xs">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="space-y-1">
              <label class="block font-bold text-slate-700">Legal Business / Organization Name <span class="text-rose-500">*</span></label>
              <input
                v-model="businessForm.business_name"
                type="text"
                required
                placeholder="e.g. Lodge of Fraternity No. 1418 Ltd"
                class="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl font-medium text-slate-900 focus:outline-none focus:border-indigo-500"
              />
            </div>

            <!-- Company / Charity Registration No. -->
            <div class="space-y-1">
              <label class="block font-bold text-slate-700">Company / Charity Registration Number</label>
              <input
            ...
            <div class="space-y-1">
              <label class="block font-bold text-slate-700">Tax / VAT Registration ID</label>
              <input
            ...
            <div class="space-y-1">
              <label class="block font-bold text-slate-700">Official Accounts / Billing Email <span class="text-rose-500">*</span></label>
              <input
            ...
            <div class="space-y-1">
              <label class="block font-bold text-slate-700">Accounts / Treasurer Phone</label>
              <input
            ...
            <div class="space-y-1">
              <label class="block font-bold text-slate-700">Registered Business / Clubhouse Address</label>
              <input
          ...
          <div class="pt-5 border-t border-slate-100">
            <div class="flex items-center justify-between mb-3">
              <div>
                ...
                <p class="text-xs text-slate-500">Provide live API keys for your club's merchant account so member payments and event ticket fees flow directly to your business bank account.</p>
              </div>
              <button
                type="button"
                @click="showSecretKeys = !showSecretKeys"
                class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-lg transition-all"
              >
                {{ showSecretKeys ? '🙈 Hide API Keys' : '👁️ Show API Keys' }}
              </button>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200/80">
              <div class="space-y-1">
                <label class="block font-bold text-slate-700">Stripe Publishable Key</label>
                <input
                  v-model="businessForm.stripe_publishable_key"
                  :type="showSecretKeys ? 'text' : 'password'"
                  placeholder="pk_live_..."
                  class="w-full p-2 bg-white border border-slate-300 rounded-lg font-mono text-[11px] text-slate-900 focus:outline-none focus:border-indigo-500"
                />
              </div>

              <div class="space-y-1">
                <label class="block font-bold text-slate-700">Stripe Secret Key</label>
                <input
              ...
              <div class="space-y-1">
                <label class="block font-bold text-slate-700">Stripe Webhook Secret</label>
                <input
          ...
          <div class="flex justify-end pt-2">
            <button
              ...
              class="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-md transition-all flex items-center gap-2 cursor-pointer"
            >
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5">
          <div class="flex items-center justify-between border-b border-slate-100 pb-4">
            <div>
              <span class="text-[10px] font-extrabold uppercase tracking-wider text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full border border-indigo-100">
                Active Platform Subscription
              </span>
              <h3 class="text-lg font-bold text-slate-900 mt-2">{{ currentPlan.name }}</h3>
              <p class="text-xs text-slate-500 mt-0.5">{{ currentPlan.billing_cycle }}</p>
            </div>
            <div class="text-right">
              <span class="text-2xl font-black text-slate-900 block">{{ currentPlan.price }}</span>
              <span class="text-[11px] font-bold text-emerald-600 flex items-center justify-end gap-1 mt-0.5">
          ...
          <div>
            <h4 class="text-xs font-bold text-slate-700 uppercase tracking-wider mb-3">Included Platform Features & Modules</h4>
            <div class="flex flex-wrap gap-2">
              <span v-for="(module, idx) in currentPlan.modules" :key="idx" class="px-3 py-1 bg-slate-100 text-slate-700 font-bold text-xs rounded-xl border border-slate-200 flex items-center gap-1.5">
          ...
          <div class="pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
            <span>Next Automated Renewal Date: <strong class="text-slate-900">{{ currentPlan.next_billing_date }}</strong></span>
          </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4 flex flex-col justify-between">
          <div>
            ...
            <p class="text-xs text-slate-500 mt-1">Select your preferred payment processor for member dues & SaaS checkouts.</p>
            ...
            <div class="space-y-2 mt-4">
              ...
              >
                ...
                <span v-if="providerForm.provider === 'stripe'" class="text-indigo-600 font-black">✓ Active</span>
              </button>
              ...
              >
                ...
                <span v-if="providerForm.provider === 'paddle'" class="text-indigo-600 font-black">✓ Active</span>
              </button>
            </div>
          </div>

          <div class="p-3 bg-slate-50 rounded-xl border border-slate-200/80 text-[11px] text-slate-500">
      ...
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden space-y-4 p-6">
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
          <div>
            <h3 class="text-base font-bold text-slate-900">📄 SaaS Platform Invoices & Receipts</h3>
            <p class="text-xs text-slate-500 mt-0.5">View recent platform subscription payments and download tax receipts.</p>
          </div>
          <button type="button" class="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl border border-slate-300 transition-all cursor-pointer">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left text-xs">
            ...
            <tbody class="divide-y divide-slate-100">
              <tr v-for="inv in billingHistory" :key="inv.id" class="hover:bg-slate-50/80 transition-colors">
                <td class="p-3 font-mono font-bold text-slate-900">{{ inv.id }}</td>
                <td class="p-3 font-medium text-slate-600">{{ inv.date }}</td>
                <td class="p-3 font-medium text-slate-800">{{ inv.description }}</td>
                <td class="p-3 font-bold text-slate-900">{{ inv.amount }}</td>
                <td class="p-3">
