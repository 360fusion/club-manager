      ...
      <div class="p-6 bg-white border border-slate-200/80 rounded-3xl shadow-sm space-y-4">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
          <div>
            ...
            <p class="text-xs text-slate-500 font-medium mt-0.5">Provincial Honor Milestone Target: £{{ Number(target.target_amount).toLocaleString('en-GB', { minimumFractionDigits: 2 }) }}</p>
          </div>
        ...
        <div class="space-y-2">
          <div class="flex items-center justify-between text-xs font-black">
            <span class="text-slate-700">Total Raised to Date: {{ formattedTotalRaised }}</span>
          ...
          <div class="grid grid-cols-4 gap-2 pt-2 text-[10px] font-bold text-center">
            <div :class="['p-3 rounded-xl border', totalRaisedForFestival >= target.bronze_tier ? 'bg-amber-100/70 border-amber-300 text-amber-950' : 'bg-slate-50 border-slate-200 text-slate-400']">
              <span>🥉 Bronze Tier</span>
              <span class="block font-black text-xs">£{{ Number(target.bronze_tier).toLocaleString('en-GB') }}</span>
            </div>
            <div :class="['p-3 rounded-xl border', totalRaisedForFestival >= target.silver_tier ? 'bg-slate-200 border-slate-400 text-slate-900' : 'bg-slate-50 border-slate-200 text-slate-400']">
              <span>🥈 Silver Tier</span>
              <span class="block font-black text-xs">£{{ Number(target.silver_tier).toLocaleString('en-GB') }}</span>
            </div>
            <div :class="['p-3 rounded-xl border', totalRaisedForFestival >= target.gold_tier ? 'bg-yellow-100 border-yellow-300 text-yellow-950' : 'bg-slate-50 border-slate-200 text-slate-400']">
              <span>🥇 Gold Tier</span>
              <span class="block font-black text-xs">£{{ Number(target.gold_tier).toLocaleString('en-GB') }}</span>
            </div>
            <div :class="['p-3 rounded-xl border', totalRaisedForFestival >= target.platinum_tier ? 'bg-indigo-100 border-indigo-300 text-indigo-950' : 'bg-slate-50 border-slate-200 text-slate-400']">
        ...
        <div class="border border-slate-200 rounded-2xl overflow-hidden shadow-2xs">
          <table class="w-full text-left border-collapse text-xs">
            ...
            <tbody class="divide-y divide-slate-100 text-slate-700">
              <tr v-for="m in activeMembers" :key="m.id" class="hover:bg-slate-50/80 transition-colors">
                <td class="py-3 px-4 font-bold text-slate-900">{{ m.name }} ({{ m.rank }})</td>
                <td class="py-3 px-4 text-right font-semibold text-slate-800">
                  {{ m.regular_giving > 0 ? '£' + m.regular_giving.toFixed(2) : '—' }}
                </td>
                <td class="py-3 px-4 text-right font-black text-purple-950">{{ m.formatted_donated }}</td>
                <td class="py-3 px-4">
                  <div class="flex items-center gap-1.5">
                    <span v-if="m.qualifies_for_jewel" class="px-2 py-0.5 bg-amber-100 text-amber-950 border border-amber-300 rounded font-black text-[10px]">
                      🏅 Festival Jewel
                    </span>
                    <span v-if="m.qualifies_for_bar" class="px-2 py-0.5 bg-indigo-100 text-indigo-950 border border-indigo-300 rounded font-black text-[10px]">
                      🎗️ Honor Bar
                    </span>
                ...
                <td class="py-3 px-4 text-right">
                  <button type="button" @click="openGivingModal(m)" class="px-3 py-1 bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-[10px] rounded-lg shadow-sm transition">
                    Edit Giving
                  </button>
                </td>
              </tr>
        ...
        <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 space-y-5 border border-slate-100">
          <div class="flex items-center justify-between pb-3 border-b border-slate-100">
            <div class="flex items-center gap-2">
              ...
              <h3 class="font-black text-slate-900 text-base">Edit Festival Target Milestones</h3>
            </div>
            <button type="button" @click="showTargetModal = false" class="text-slate-400 hover:text-slate-600 text-lg">✕</button>
          </div>

          <form @submit.prevent="submitTarget" class="space-y-4 text-xs">
            <div>
              <label class="font-bold text-slate-700 block mb-1">Festival Name *</label>
              <input type="text" v-model="targetForm.festival_name" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-500" required />
            </div>

            <div>
              <label class="font-bold text-slate-700 block mb-1">MCF Relief Chest Reference *</label>
              <input type="text" v-model="targetForm.relief_chest_ref" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-500" required />
            </div>

            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="font-bold text-slate-700 block mb-1">Bronze Tier (£)</label>
                <input type="number" step="0.01" v-model="targetForm.bronze_tier" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-500" required />
              </div>
              <div>
                <label class="font-bold text-slate-700 block mb-1">Silver Tier (£)</label>
                <input type="number" step="0.01" v-model="targetForm.silver_tier" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-500" required />
              </div>
            </div>

            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="font-bold text-slate-700 block mb-1">Gold Tier (£)</label>
                <input type="number" step="0.01" v-model="targetForm.gold_tier" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-500" required />
              </div>
              <div>
                <label class="font-bold text-slate-700 block mb-1">Platinum Tier (£)</label>
                <input type="number" step="0.01" v-model="targetForm.platinum_tier" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-500" required />
              </div>
            </div>

            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
              <button type="button" @click="showTargetModal = false" class="px-4 py-2 bg-slate-100 text-slate-700 font-bold rounded-xl hover:bg-slate-200 transition">Cancel</button>
              <button type="submit" class="px-5 py-2 bg-purple-700 hover:bg-purple-800 text-white font-black rounded-xl shadow-md transition">Save Milestones</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div v-if="showGivingModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
        <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 space-y-5 border border-slate-100">
          <div class="flex items-center justify-between pb-3 border-b border-slate-100">
            ...
            <button type="button" @click="showGivingModal = false" class="text-slate-400 hover:text-slate-600 text-lg">✕</button>
          </div>
          ...
          <form @submit.prevent="submitGiving" class="space-y-4 text-xs">
            <div>
              ...
              <input type="number" step="0.01" v-model="givingForm.regular_giving_amount" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500" />
            </div>

            <div>
              <label class="font-bold text-slate-700 block mb-1">Total Donated to Date (£) *</label>
              <input type="number" step="0.01" v-model="givingForm.total_donated_to_date" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500" required />
            </div>
            ...
            <div class="space-y-2 p-3 bg-indigo-50 border border-indigo-200 rounded-xl">
              <label class="flex items-center gap-2 font-bold text-indigo-950 cursor-pointer">
                <input type="checkbox" v-model="givingForm.qualifies_for_jewel" class="rounded text-indigo-600 focus:ring-indigo-500" />
                <span>Qualifies for Festival Jewel (&gt;= £250)</span>
              </label>

              <label class="flex items-center gap-2 font-bold text-indigo-950 cursor-pointer">
                <input type="checkbox" v-model="givingForm.qualifies_for_bar" class="rounded text-indigo-600 focus:ring-indigo-500" />
                <span>Qualifies for Honor Bar (&gt;= £500)</span>
              </label>
            </div>

            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
              <button type="button" @click="showGivingModal = false" class="px-4 py-2 bg-slate-100 text-slate-700 font-bold rounded-xl hover:bg-slate-200 transition">Cancel</button>
