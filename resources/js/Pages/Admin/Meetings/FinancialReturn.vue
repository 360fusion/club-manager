...
<template>
  <AdminLayout :club="club" title="Meeting Financial Return">
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          <div class="flex items-center gap-2 text-xs text-slate-500 font-semibold mb-1">
            <Link :href="route('admin.meetings.index', club.slug)" class="hover:text-slate-800 transition-colors">Meetings</Link>
            <span>/</span>
            <Link :href="route('admin.meetings.show', { clubSlug: club.slug, id: meeting.id })" class="hover:text-slate-800 transition-colors">
        ...
        <Link
          ...
          class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl shadow-sm transition-all flex items-center gap-1.5 self-start sm:self-auto"
        >
      ...
      <div class="bg-slate-900 text-white p-5 rounded-2xl shadow-lg border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="space-y-1">
