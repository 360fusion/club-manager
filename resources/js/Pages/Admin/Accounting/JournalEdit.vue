      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <nav class="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-1" aria-label="Breadcrumb">
            ...
            <span>/</span>
            <Link :href="route('admin.accounting.index', { clubSlug: club.slug, tab: 'general_ledger' })" class="hover:text-slate-900 transition-colors">
              General Ledger
            </Link>
            <span>/</span>
            <span class="text-slate-900 font-bold" aria-current="page">Edit {{ journalEntry.reference_number }}</span>
          </nav>
          <h1 class="text-2xl font-black text-slate-900 tracking-tight">Edit Journal Entry {{ journalEntry.reference_number }}</h1>
          <p class="text-sm text-slate-500 mt-1">Modify multi-line double-entry journal items and ledger allocations.</p>
        </div>
        <Link
        ...
        <div class="p-6 sm:p-8 space-y-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="space-y-1 sm:col-span-2">
              <label class="block text-sm font-bold text-slate-700">Description / Memo <span class="text-rose-500">*</span></label>
              <input
            ...
            <div class="space-y-1">
              <label class="block text-sm font-bold text-slate-700">Entry Date <span class="text-rose-500">*</span></label>
              <input
          ...
          <div class="space-y-4">
            <div class="flex items-center justify-between">
              ...
              <button
                type="button"
                @click="addLine"
                class="inline-flex items-center gap-1 text-xs font-extrabold text-sky-600 hover:text-sky-800 transition-colors"
              >
                + Add Line Item
              </button>
            </div>
            ...
            <div class="overflow-x-auto border border-slate-200 rounded-2xl">
              <table class="w-full text-left border-collapse min-w-[640px]">
                <thead>
                  <tr class="bg-slate-50 border-b border-slate-200 text-[11px] font-black uppercase text-slate-500 tracking-wider">
                  ...
                  <tr v-for="(item, idx) in form.items" :key="idx" class="hover:bg-slate-50/50">
                    <td class="p-2">
                      <select
                        ...
                        required
                        class="w-full px-3 py-1.5 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 font-medium"
                      >
                    ...
                    <td class="p-2">
                      <input
                        ...
                        class="w-full px-3 py-1.5 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900"
                      />
                    </td>
                    <td class="p-2">
                      <input
                        ...
                        min="0"
                        placeholder="0.00"
                        class="w-full px-3 py-1.5 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 text-right font-mono"
                      />
                    </td>
                    <td class="p-2">
                      <input
                    ...
                    <td class="p-2 text-center">
                      <button
                        ...
                        class="text-slate-400 hover:text-rose-600 disabled:opacity-20 transition-colors p-1"
                      >
                        ✕
                      </button>
                    </td>
                ...
                <tfoot>
                  <tr class="bg-slate-50 font-bold text-xs border-t border-slate-200">
                    <td colspan="2" class="p-3 text-right text-slate-600 uppercase tracking-wider">Total:</td>
                    <td class="p-3 text-right font-mono text-slate-900">£{{ totalDebit.toFixed(2) }}</td>
                    <td class="p-3 text-right font-mono text-slate-900">£{{ totalCredit.toFixed(2) }}</td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
            ...
            <div
              ...
              :class="{
                'bg-emerald-50 border-emerald-200 text-emerald-900': isBalanced,
                'bg-rose-50 border-rose-200 text-rose-900': !isBalanced
              }"
            >
