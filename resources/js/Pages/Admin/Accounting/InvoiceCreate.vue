...
<template>
  <AdminLayout :club="club" title="Create Invoice" active-tab="accounting">
    ...
    <div class="max-w-3xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <nav class="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1" aria-label="Breadcrumb">
            ...
            <span>/</span>
            <Link :href="route('admin.accounting.index', { clubSlug: club.slug, tab: 'sales' })" class="hover:text-slate-900 dark:hover:text-white transition-colors">
              Sales
            </Link>
            <span>/</span>
            <span class="text-slate-900 dark:text-white font-bold" aria-current="page">Create Invoice</span>
          </nav>
          <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Create Invoice</h1>
          <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Issue an invoice to a member for dues, fees, or other charges.</p>
        </div>
        <Link
      ...
      <div class="bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 rounded-2xl p-4 flex gap-3">
      ...
      <form @submit.prevent="submit" class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <div class="p-6 sm:p-8 space-y-6">
          ...
          <div class="space-y-4">
            ...
            <!-- Member search -->
            <div>
              <label class="block text-xs font-bold text-slate-800 dark:text-slate-100 mb-1">
              ...
              <input
                ...
                class="w-full px-3.5 py-2.5 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-medium text-slate-900 dark:text-white bg-slate-50 dark:bg-slate-800/50 focus:outline-none focus:ring-2 focus:ring-blue-400 mb-2"
              />
              ...
              <div class="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden max-h-48 overflow-y-auto">
                <label
                  ...
                  :class="[
                    ...
                    form.user_id == m.id ? 'bg-blue-50 dark:bg-blue-950/40 border-l-2 border-blue-500' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50 border-l-2 border-transparent'
                  ]"
                >
                  <input
                    type="radio"
                  ...
                  />
                  <div>
                    <span class="text-xs font-bold text-slate-900 dark:text-white">{{ m.name }}</span>
              ...
              <p v-if="form.errors.user_id" class="text-xs text-red-600 dark:text-red-400 font-semibold mt-1">{{ form.errors.user_id }}</p>
            </div>

            <!-- Selected member preview -->
            <div v-if="selectedMember" class="flex items-center gap-3 px-4 py-3 bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 rounded-xl">
              <span class="text-xl">👤</span>
              <div>
                <p class="text-xs font-extrabold text-blue-900 dark:text-blue-200">{{ selectedMember.name }}</p>
                <p class="text-[11px] text-blue-600 dark:text-blue-400 font-mono">{{ selectedMember.email }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <!-- Invoice number preview -->
            <div class="flex items-center gap-2 px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded-xl sm:w-1/2">
              <span class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">Invoice #</span>
              <span class="text-xs font-black font-mono text-slate-600 dark:text-slate-300">{{ previewNumber }}</span>
              <span class="text-[10px] text-slate-400 italic">(auto-assigned)</span>
            </div>

            <!-- Title / Description -->
            <div>
              <label for="invoice-title" class="block text-xs font-bold text-slate-800 dark:text-slate-100 mb-1">
              ...
              <input
                ...
                required
              />
              <p v-if="form.errors.title" class="text-xs text-red-600 dark:text-red-400 font-semibold mt-1">{{ form.errors.title }}</p>
            </div>
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <input
                    ...
                    min="0.01"
                    placeholder="0.00"
                    class="w-full pl-7 pr-3.5 py-2.5 border border-slate-300 dark:border-slate-700 rounded-xl text-xs font-mono font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    required
                  />
                </div>
                <p v-if="form.errors.amount" class="text-xs text-red-600 dark:text-red-400 font-semibold mt-1">{{ form.errors.amount }}</p>
              </div>

              <div>
                <label for="invoice-due" class="block text-xs font-bold text-slate-800 dark:text-slate-100 mb-1">Due Date</label>
                <input
                  id="invoice-due"
          ...
          <div class="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <div>
              <label for="invoice-attachment" class="block text-xs font-bold text-slate-800 dark:text-slate-100 mb-1">
              ...
              <input
                ...
                class="w-full text-xs text-slate-600 dark:text-slate-300 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-slate-100 dark:file:bg-slate-800 file:text-slate-700 dark:file:text-slate-200 hover:file:bg-slate-200 dark:hover:file:bg-slate-700 cursor-pointer"
              />
              <p v-if="form.errors.attachment" class="text-xs text-red-600 dark:text-red-400 font-semibold mt-1">{{ form.errors.attachment }}</p>
              <p v-if="form.attachment" class="text-[11px] text-emerald-600 dark:text-emerald-400 font-bold mt-1">
          ...
          <div class="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 text-xs text-slate-600 dark:text-slate-300 space-y-1">
        ...
        <div class="bg-slate-50 dark:bg-slate-800/50 px-6 py-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            <button
              ...
              class="px-5 py-2.5 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 disabled:opacity-40 disabled:cursor-not-allowed text-slate-800 dark:text-slate-100 font-extrabold text-xs rounded-xl transition-colors cursor-pointer flex items-center gap-1.5"
            >
