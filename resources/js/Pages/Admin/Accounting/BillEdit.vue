      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <nav class="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-1" aria-label="Breadcrumb">
            ...
            <span>/</span>
            <span class="text-slate-900 font-bold" aria-current="page">Edit {{ bill.bill_number }}</span>
          </nav>
          <div class="flex items-center gap-3">
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Edit Vendor Bill {{ bill.bill_number }}</h1>
            <span
              class="px-2.5 py-0.5 text-xs font-black uppercase tracking-wider rounded-full"
              :class="{
                'bg-emerald-100 text-emerald-800': bill.status === 'paid',
                'bg-amber-100 text-amber-800': bill.status === 'unpaid',
          ...
          <p class="text-sm text-slate-500 mt-1">Update vendor bill details, category, due date, or status.</p>
        </div>
        <Link
        ...
        <div class="p-6 sm:p-8 space-y-6">
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div class="space-y-1 sm:col-span-2">
                <label class="block text-sm font-bold text-slate-700">Notes & Reference</label>
                <textarea
                  v-model="form.notes"
                  rows="3"
            ...
            <div class="space-y-2 pt-2">
              <label class="block text-sm font-bold text-slate-700">Replace / Upload Invoice Document</label>
              <div v-if="bill.attachment_url" class="text-xs text-slate-600 mb-1">
                Current attachment: <a :href="bill.attachment_url" target="_blank" class="text-sky-600 underline font-semibold">View File</a>
              </div>
              <input
                type="file"
