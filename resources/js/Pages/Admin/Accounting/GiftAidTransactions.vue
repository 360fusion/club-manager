...
<template>
  <Head :title="`Charity Gift Aid Transactions — ${club.name}`" />
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 dark:bg-slate-700 text-white p-6 rounded-3xl shadow-xl">
        <div class="space-y-1">
          <div class="flex items-center gap-2.5">
            <Link
              :href="route('admin.club_acc.charity.index', club.slug)"
              class="p-2 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-xl text-xs font-bold transition flex items-center gap-1"
            >
              <span>← Charity Dashboard</span>
            </Link>
          ...
          <h1 class="text-2xl font-black tracking-tight flex items-center gap-2 pt-1">
            <span>🏛️ Charity Gift Aid Transactions</span>
          </h1>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 text-xs">
            ...
            <div>
              ...
              >
                ...
                <option
                  v-for="m in (reconciledDonations?.members || [])"
                  :key="m.id"
                  :value="m.id"
                >
                  {{ m.name }}
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800 font-medium text-slate-800 dark:text-slate-100">
              ...
              <tr v-if="filteredCollections.length === 0">
                <td colspan="8" class="py-12 text-center text-slate-400 italic">
                  No charity gift aid transactions match the selected filter criteria.
                </td>
              </tr>
            </tbody>
          </table>
