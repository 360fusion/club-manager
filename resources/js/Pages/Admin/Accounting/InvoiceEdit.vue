      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <nav class="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1" aria-label="Breadcrumb">
            ...
            <span>/</span>
            <span class="text-slate-900 dark:text-white font-bold" aria-current="page">Edit {{ invoice.invoice_number }}</span>
          </nav>
          <div class="flex items-center gap-3">
            <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Edit Invoice {{ invoice.invoice_number }}</h1>
            <span
      ...
      <form @submit.prevent="submit" class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <div class="p-6 sm:p-8 space-y-6">
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-2">
              <label class="block text-sm font-bold text-slate-700 dark:text-slate-200">Select Member <span class="text-rose-500">*</span></label>
              <input
              ...
              <select
                ...
                required
                size="4"
                class="w-full px-4 py-2.5 text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 dark:focus:ring-slate-400"
              >
            ...
            <div v-if="selectedMember" class="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-800 text-xs flex items-center justify-between">
              <div>
                <span class="font-bold text-slate-900 dark:text-white">{{ selectedMember.name }}</span>
                <span class="text-slate-500 dark:text-slate-400 ml-2">{{ selectedMember.email }}</span>
              </div>
              <span class="text-emerald-700 dark:text-emerald-300 font-semibold bg-emerald-50 dark:bg-emerald-950/40 px-2 py-0.5 rounded-md border border-emerald-200 dark:border-emerald-800/60">Selected</span>
            </div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div class="space-y-1 sm:col-span-2">
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-200">Invoice Title / Description <span class="text-rose-500">*</span></label>
                <input
                  v-model="form.title"
                  type="text"
                  required
            ...
            <div class="space-y-2 pt-2">
              <label class="block text-sm font-bold text-slate-700 dark:text-slate-200">Replace / Add Supporting Document</label>
              <div v-if="invoice.attachment_url" class="text-xs text-slate-600 dark:text-slate-300 mb-1">
                Current attachment: <a :href="invoice.attachment_url" target="_blank" class="text-blue-600 dark:text-blue-400 underline font-semibold">View File</a>
              </div>
              <input
                type="file"
