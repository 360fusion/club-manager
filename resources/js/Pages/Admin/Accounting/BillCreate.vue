...
<template>
  <AdminLayout :club="club" title="Add Vendor Bill" active-tab="accounting">
    ...
    <div class="max-w-3xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <nav class="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-1" aria-label="Breadcrumb">
            ...
            <span>/</span>
            <Link :href="route('admin.accounting.index', { clubSlug: club.slug, tab: 'purchases' })" class="hover:text-slate-900 transition-colors">
              Purchases
            </Link>
            <span>/</span>
            <span class="text-slate-900 font-bold" aria-current="page">Add Vendor Bill</span>
          </nav>
          <h1 class="text-2xl font-black text-slate-900 tracking-tight">Add Vendor Bill (A/P)</h1>
          <p class="text-sm text-slate-500 mt-1">Record a supplier or vendor bill to manage Accounts Payable.</p>
        </div>
        <Link
          :href="route('admin.accounting.index', { clubSlug: club.slug, tab: 'purchases' })"
          class="inline-flex items-center gap-1 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition-colors shrink-0"
        >
          ← Back to Purchases
        </Link>
      </div>
      ...
      <!-- Form -->
      <form @submit.prevent="submit(false)" class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-6 sm:p-8 space-y-6">
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div class="space-y-1">
                <label class="block text-sm font-bold text-slate-700">Vendor / Supplier Name <span class="text-rose-500">*</span></label>
                <input
              ...
              <div class="space-y-1">
                <label class="block text-sm font-bold text-slate-700">Category / Account <span class="text-rose-500">*</span></label>
                <select
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div class="space-y-1">
                <label class="block text-sm font-bold text-slate-700">Total Amount (£) <span class="text-rose-500">*</span></label>
                <div class="relative">
                  ...
                  <input
                    ...
                    required
                    placeholder="0.00"
                    class="w-full pl-8 pr-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 font-semibold"
                  />
                </div>
              ...
              <div class="space-y-1">
                <label class="block text-sm font-bold text-slate-700">Due Date <span class="text-rose-500">*</span></label>
                <input
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-1">
              <label class="block text-sm font-bold text-slate-700">Internal Notes / Description</label>
              <textarea
                v-model="form.notes"
                rows="3"
            ...
            <div class="space-y-1">
              <label class="block text-sm font-bold text-slate-700">Attach Supplier Invoice / Receipt (PDF or Image)</label>
              <input
                type="file"
                accept=".pdf,.png,.jpg,.jpeg,.webp"
                @change="handleFileChange"
                class="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200"
              />
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3">
          <Link
            :href="route('admin.accounting.index', { clubSlug: club.slug, tab: 'purchases' })"
            class="w-full sm:w-auto px-5 py-2.5 text-center text-sm font-bold text-slate-600 hover:text-slate-900 transition-colors"
          >
            Cancel
          </Link>
          <div class="flex items-center gap-3 w-full sm:w-auto">
            <button
              ...
              @click="submit(true)"
              class="flex-1 sm:flex-none px-5 py-2.5 bg-white border border-slate-300 hover:bg-slate-100 text-slate-700 font-bold text-sm rounded-xl transition-all disabled:opacity-50"
            >
              Save as Draft
            </button>
            <button
              type="submit"
              :disabled="form.processing"
              class="flex-1 sm:flex-none px-6 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-sm rounded-xl transition-all shadow-md shadow-slate-900/10 disabled:opacity-50"
            >
