  ...
  <AdminLayout :club="club" title="Post Journal Entry" active-tab="accounting">
    ...
    <div class="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <nav class="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-1" aria-label="Breadcrumb">
            ...
            <span>/</span>
            <Link :href="route('admin.accounting.index', { clubSlug: club.slug, tab: 'accounting' })" class="hover:text-slate-900 transition-colors">
              General Ledger
            </Link>
            <span>/</span>
            <span class="text-slate-900 font-bold" aria-current="page">Post Journal Entry</span>
          </nav>
          <h1 class="text-2xl font-black text-slate-900 tracking-tight">Post Manual Journal Entry</h1>
          <p class="text-sm text-slate-500 mt-1">Create double-entry ledger postings with balanced debits and credits.</p>
        </div>
        <Link
      ...
      <div
        ...
        :class="isBalanced ? 'bg-emerald-50 border-emerald-200 text-emerald-900' : 'bg-amber-50 border-amber-200 text-amber-900'"
      >
        ...
        <div class="flex items-center gap-4 text-xs font-bold shrink-0">
          <div>Debits: <span class="font-mono text-sm text-slate-900">£{{ totalDebit.toFixed(2) }}</span></div>
          <div>Credits: <span class="font-mono text-sm text-slate-900">£{{ totalCredit.toFixed(2) }}</span></div>
        </div>
      </div>
      ...
      <form @submit.prevent="submit" class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-6 sm:p-8 space-y-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="sm:col-span-2 space-y-1">
              <label class="block text-sm font-bold text-slate-700">Journal Description / Memo <span class="text-rose-500">*</span></label>
              <input
            ...
            <div class="space-y-1">
              <label class="block text-sm font-bold text-slate-700">Posting Date <span class="text-rose-500">*</span></label>
              <input
            ...
            <div class="border border-slate-200 rounded-2xl overflow-hidden">
              <table class="w-full text-left text-xs">
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                  <tr v-for="(item, index) in form.items" :key="index" class="hover:bg-slate-50/50">
                    <td class="p-3">
                      <select
                        v-model="item.account_id"
                        required
                        class="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-slate-900"
                      >
                    ...
                    <td class="p-3">
                      <input
                        ...
                        class="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-right font-mono focus:outline-none focus:ring-1 focus:ring-slate-900"
                      />
                    </td>
                    <td class="p-3">
                      <input
                    ...
                    <td class="p-3 text-center">
                      <button
                        ...
                        class="text-slate-400 hover:text-rose-600 disabled:opacity-30 transition-colors"
                      >
                        ✕
                      </button>
                    </td>
                ...
                <tfoot class="bg-slate-50 font-bold border-t border-slate-200">
                  <tr>
                    <td colspan="2" class="py-3 px-4 text-right text-slate-600">Totals:</td>
                    <td class="py-3 px-4 text-right font-mono text-slate-900">£{{ totalDebit.toFixed(2) }}</td>
                    <td class="py-3 px-4 text-right font-mono text-slate-900">£{{ totalCredit.toFixed(2) }}</td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
        ...
        <!-- Footer Actions -->
        <div class="p-6 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-3">
          <Link
            :href="route('admin.accounting.index', { clubSlug: club.slug, tab: 'accounting' })"
          ...
          <button
            ...
            class="px-6 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-sm rounded-xl transition-all shadow-md shadow-slate-900/10 disabled:opacity-50 disabled:cursor-not-allowed"
          >
