...
<template>
  <MemberLayout title="Member Portal" :club="club" :member-role="memberRole" active-tab="dashboard">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <!-- Left Column: Upcoming Events & RSVPs (2 cols) -->
      <div id="events-section" class="lg:col-span-2 space-y-6">
        <div class="flex items-center justify-between">
          <h2 class="text-xl font-bold text-slate-900">📅 Upcoming Events & Training Sessions</h2>
        ...
        <div v-if="events.length" class="space-y-4">
          <div v-for="event in events" :key="event.id" class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/80 hover:border-slate-300 transition-all space-y-4">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <button @click="openRsvpModal(event)" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow-md shadow-emerald-600/20 transition-all self-start sm:self-auto">
