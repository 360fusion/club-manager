    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200/80 dark:border-slate-800/80">
        <div>
          <div class="flex items-center gap-3">
            <h2 class="text-xl font-bold text-slate-900 dark:text-white">My Joined Clubs</h2>
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Select any of your sports or community clubs below to view member events, RSVPs, and dues.</p>
        </div>
      ...
      <div class="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200/80 dark:border-slate-800/80 flex flex-col sm:flex-row gap-3 items-center justify-between">
        <div class="relative w-full sm:w-80">
          ...
          <input
            ...
            class="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 text-xs rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all"
          />
        </div>

        <div class="text-xs text-slate-500 dark:text-slate-400 font-semibold">
          Showing <strong class="text-slate-900 dark:text-white">{{ filteredClubs.length }}</strong> of <strong class="text-slate-900 dark:text-white">{{ clubs.length }}</strong> clubs
        </div>
      </div>
      ...
      <div v-if="filteredClubs.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
          ...
          :key="c.id"
          :class="[
            'bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border transition-all flex flex-col justify-between space-y-6 group',
            c.slug === club.slug ? 'border-emerald-500 ring-2 ring-emerald-500/20' : 'border-slate-200/80 dark:border-slate-800/80 hover:border-emerald-300 dark:hover:border-emerald-700/60'
          ]"
        >
          <div class="space-y-4">
            ...
            <div>
              <div class="flex items-center gap-2">
                <h3 class="text-lg font-bold text-slate-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{{ c.name }}</h3>
                <span v-if="c.slug === club.slug" class="px-2 py-0.2 rounded text-[9px] font-extrabold uppercase bg-emerald-600 text-white">
                  Current
                </span>
              </div>
              <div class="flex items-center gap-2 mt-2">
                <span v-if="c.member_number" class="text-xs font-mono font-semibold px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-md border border-slate-200 dark:border-slate-800">
                  {{ c.member_number }}
                </span>
          ...
          <div class="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
            ...
            <Link
              ...
              class="py-2.5 px-3 bg-blue-50 dark:bg-blue-950/40 hover:bg-blue-100 dark:hover:bg-blue-900/40 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800/60 text-xs font-bold rounded-xl transition-all"
            >
              Admin &rarr;
            </Link>
          </div>
      ...
      <div v-else class="bg-white dark:bg-slate-900 rounded-2xl p-12 text-center border border-slate-200/80 dark:border-slate-800/80 text-slate-500 dark:text-slate-400 text-xs space-y-3">
