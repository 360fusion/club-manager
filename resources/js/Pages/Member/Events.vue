...
<template>
  <MemberLayout title="My Events & RSVPs" :club="club" :member-role="memberRole" active-tab="events">
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200/80">
        <div>
          <div class="flex items-center gap-3 flex-wrap">
            <h2 class="text-xl font-bold text-slate-900">Meetings, Events & Training Schedule</h2>
          ...
          <p class="text-xs text-slate-500 mt-1">View official lodge meeting summons, confirm attendance & dining RSVPs, select 3-course menus, and access summons PDFs.</p>
        </div>

        <!-- Filter Tab Pill Buttons -->
        <div class="flex items-center gap-1.5 p-1 bg-slate-100 rounded-xl self-start md:self-auto text-xs font-bold">
          <button
            @click="activeFilter = 'all'"
            :class="['px-3 py-1.5 rounded-lg transition-all cursor-pointer', activeFilter === 'all' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900']"
          >
            All ({{ totalScheduleCount }})
          </button>
          <button
            @click="activeFilter = 'meetings'"
            :class="['px-3 py-1.5 rounded-lg transition-all cursor-pointer flex items-center gap-1', activeFilter === 'meetings' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900']"
          >
            📜 Meetings ({{ meetings.length }})
          </button>
          <button
            @click="activeFilter = 'events'"
            :class="['px-3 py-1.5 rounded-lg transition-all cursor-pointer flex items-center gap-1', activeFilter === 'events' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900']"
          >
      ...
      <div v-if="(activeFilter === 'all' || activeFilter === 'meetings') && meetings.length > 0" class="space-y-4">
        <div class="flex items-center justify-between">
          <h3 class="text-base font-extrabold text-slate-900 flex items-center gap-2">
            ...
            <span class="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">{{ meetings.length }}</span>
          </h3>
        </div>
            ...
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              <div class="space-y-2">
                <div class="flex items-center gap-2 flex-wrap">
                  ...
                  <span v-if="meeting.user_rsvp?.attendance_status === 'attending_dining'" class="px-2.5 py-1 rounded-lg text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
                    🟢 Attending Dining
                  </span>
                  <span v-else-if="meeting.user_rsvp?.attendance_status === 'attending_meeting_only'" class="px-2.5 py-1 rounded-lg text-xs font-bold bg-sky-50 text-sky-700 border border-sky-200">
                    🔵 Attending Meeting Only
                  </span>
                  <span v-else-if="meeting.user_rsvp?.attendance_status === 'apologies'" class="px-2.5 py-1 rounded-lg text-xs font-bold bg-rose-50 text-rose-700 border border-rose-200">
                    🔴 Apologies Sent
                  </span>
                  <span v-else class="px-2.5 py-1 rounded-lg text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200">
                    ⚠️ Response Required
                  </span>

                  <!-- Payment Status Badge if dining -->
                  <span v-if="meeting.user_rsvp && meeting.user_rsvp.attendance_status === 'attending_dining'" class="px-2 py-0.5 rounded text-xs font-bold" :class="meeting.user_rsvp.payment_status === 'paid' ? 'bg-emerald-100 text-emerald-800' : (meeting.user_rsvp.payment_status === 'waived' ? 'bg-purple-100 text-purple-800' : 'bg-amber-100 text-amber-800')">
                    {{ meeting.user_rsvp.payment_status === 'paid' ? '✅ PAID' : (meeting.user_rsvp.payment_status === 'waived' ? '🎁 WAIVED' : '💳 UNPAID') }}
                  </span>

                  <!-- Dining Cutoff Badge -->
                  <span v-if="meeting.rsvp_cutoff_at" class="px-2 py-0.5 rounded text-xs font-semibold" :class="meeting.is_cutoff_passed ? 'bg-rose-50 text-rose-600 border border-rose-200' : 'bg-slate-100 text-slate-600'">
                    ⏰ Cutoff: {{ meeting.rsvp_cutoff_at }}
                  </span>
                </div>

                <h4 class="text-lg font-bold text-slate-900">{{ meeting.title || 'Regular Meeting' }}</h4>
              ...
              <div class="flex flex-wrap items-center gap-2 self-start lg:self-center">
                <Link
                  ...
                  class="px-3.5 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold text-xs rounded-xl border border-indigo-200 transition-all flex items-center gap-1.5"
                >
                  📜 View Summons
                </Link>
                ...
                <a
                  ...
                  target="_blank"
                  class="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition-all flex items-center gap-1.5"
                >
                  🖨️ PDF
                </a>
                ...
                <button
                  ...
                  class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl shadow-md shadow-indigo-600/20 transition-all cursor-pointer flex items-center gap-1.5"
                >
      ...
      <div v-if="(activeFilter === 'all' || activeFilter === 'events') && events.length > 0" class="space-y-4">
        <div class="flex items-center justify-between pt-2">
          <h3 class="text-base font-extrabold text-slate-900 flex items-center gap-2">
            ...
            <span class="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">{{ events.length }}</span>
          </h3>
        </div>
      ...
      <div v-if="totalScheduleCount === 0" class="bg-white rounded-2xl p-12 text-center border border-slate-200/80 text-slate-500 text-sm">
    ...
    <div v-if="showDinnerModal && selectedEvent" class="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full space-y-6 shadow-2xl border border-slate-200">
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
          ...
          <button @click="showDinnerModal = false" class="text-slate-400 hover:text-slate-600 font-bold text-lg cursor-pointer">&times;</button>
        </div>
        ...
        <form @submit.prevent="submitRsvp" class="space-y-4">
          ...
          <div class="flex items-center gap-3 pt-4">
            <button type="submit" :disabled="rsvpForm.processing" class="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow-md shadow-emerald-600/20 transition-all cursor-pointer">
              Confirm RSVP
            </button>
            <button type="button" @click="showDinnerModal = false" class="px-4 py-3 bg-slate-100 text-slate-700 font-bold text-xs rounded-xl border border-slate-200 cursor-pointer">
              Cancel
            </button>
          </div>
        </form>
    ...
    <div v-if="showMeetingRsvpModal && selectedMeeting" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
      <div class="bg-white rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-4 my-8">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
          ...
          <button @click="showMeetingRsvpModal = false" class="text-slate-400 hover:text-slate-600 text-lg font-bold cursor-pointer">&times;</button>
        </div>
        ...
        <form @submit.prevent="submitMeetingRsvp" class="space-y-4">
          ...
          <div class="flex justify-end gap-3 pt-3 border-t border-slate-100">
            <button type="button" @click="showMeetingRsvpModal = false" class="px-4 py-2 bg-slate-100 text-slate-700 text-xs font-semibold rounded-xl cursor-pointer">Cancel</button>
