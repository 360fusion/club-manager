    ...
    <div v-if="showDinnerModal && selectedEvent" class="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 max-w-lg w-full space-y-6 shadow-2xl border border-slate-200 dark:border-slate-800">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
          ...
          <button @click="showDinnerModal = false" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 font-bold text-lg cursor-pointer">&times;</button>
        </div>
        ...
        <form @submit.prevent="submitRsvp" class="space-y-4">
          <div>
            ...
            <select v-model="rsvpForm.attendance_status" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500">
          ...
          <div v-if="selectedEvent.has_dining" class="space-y-3 pt-2 border-t border-slate-100 dark:border-slate-800">
            <div class="flex items-center gap-3">
              <input v-model="rsvpForm.attending_dining" type="checkbox" id="attending_dining_modal" class="w-4 h-4 rounded text-emerald-600 dark:text-emerald-400 border-slate-300 dark:border-slate-700" />
              <label for="attending_dining_modal" class="font-bold text-slate-900 dark:text-white text-xs">Attending Formal 3-Course Dinner</label>
            </div>
            ...
            <div v-if="rsvpForm.attending_dining" class="space-y-3 pl-2">
              ...
              <div>
                ...
                <input v-model="rsvpForm.menu_selections.main" type="text" placeholder="e.g. Pan-Seared Duck Breast" class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-white" />
              </div>
              <div>
          ...
          <div class="flex items-center gap-3 pt-4">
            ...
            <button type="button" @click="showDinnerModal = false" class="px-4 py-3 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold text-xs rounded-xl border border-slate-200 dark:border-slate-800 cursor-pointer">
              Cancel
            </button>
          </div>
        </form>
