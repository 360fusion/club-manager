    ...
    <div class="space-y-8">
      ...
      <div class="bg-gradient-to-r from-white via-blue-50 to-blue-50 p-8 rounded-3xl border border-blue-200 shadow-2xl text-slate-900 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-blue-200 pb-4">
          <div class="flex items-center gap-2 text-xs font-semibold text-slate-700">
            ...
            <span>/</span>
            <Link :href="route('superadmin.club_types.index')" class="hover:text-slate-900 transition">Masonic Orders</Link>
            <span>/</span>
            <span class="text-blue-700 font-bold">{{ clubType.name }}</span>
          </div>

          <!-- Quick Order Switcher Select -->
          <div class="flex items-center gap-2">
            <label class="text-xs font-bold text-slate-700 whitespace-nowrap">Switch Order:</label>
            <select
              :value="clubType.id"
              @change="handleSwitchOrder"
              class="bg-white border border-blue-300 text-slate-900 text-xs font-bold rounded-xl px-3 py-1.5 focus:ring-2 focus:ring-blue-500 cursor-pointer"
            >
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div class="space-y-2">
            ...
            <h1 class="text-3xl sm:text-4xl font-black tracking-tight text-slate-900">{{ clubType.name }}</h1>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <Link
              ...
              class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-800 font-black text-xs rounded-xl border border-slate-300 transition flex items-center gap-2"
            >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-7 space-y-6">
          ...
          <div v-if="clubType.website_url" class="bg-white/90 border border-slate-200 rounded-3xl p-6 shadow-xl space-y-3">
            <h3 class="text-sm font-black text-blue-600 uppercase tracking-wider flex items-center gap-2">
              <span>🏛️</span>
              <span>Official Headquarters &amp; Governing Body</span>
            </h3>
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-100 p-4 rounded-2xl border border-slate-200">
              <div>
                <span class="text-[10px] text-slate-500 font-bold block uppercase tracking-wider">Official Web Domain</span>
                <span class="text-xs text-blue-700 font-mono font-bold block mt-0.5">{{ clubType.website_url }}</span>
              </div>
              <a
                :href="clubType.website_url"
                target="_blank"
          ...
          <div class="bg-white/90 border border-slate-200 rounded-3xl p-6 shadow-xl space-y-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              <div class="bg-slate-100 p-3.5 rounded-2xl border border-slate-200 space-y-1">
                <span class="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Entity</span>
                <span class="font-extrabold text-slate-900 text-sm block">{{ clubType.terminology?.club || 'Lodge' }}</span>
              </div>

              <div class="bg-slate-100 p-3.5 rounded-2xl border border-slate-200 space-y-1">
                <span class="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Presiding Officer</span>
                <span class="font-extrabold text-slate-900 text-sm block">{{ clubType.terminology?.master || 'Worshipful Master' }}</span>
              </div>

              <div class="bg-slate-100 p-3.5 rounded-2xl border border-slate-200 space-y-1">
                <span class="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Summons Circular</span>
                <span class="font-extrabold text-slate-900 text-sm block">{{ clubType.terminology?.summons || 'Summons' }}</span>
              </div>

              <div class="bg-slate-100 p-3.5 rounded-2xl border border-slate-200 space-y-1">
                <span class="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Members</span>
                <span class="font-extrabold text-slate-900 text-sm block">{{ clubType.terminology?.members || 'Brethren' }}</span>
              </div>
            </div>
          </div>

          <!-- Provincial Rulers Schema -->
          <div v-if="clubType.rulers_schema?.length" class="bg-white/90 border border-slate-200 rounded-3xl p-6 shadow-xl space-y-3">
            ...
            <p class="text-xs text-slate-500 font-medium">Executive ruler titles assigned to Provincial Officers for clubs of this Order.</p>

            <div class="flex flex-wrap gap-2 pt-1">
              <span
                v-for="(ruler, idx) in clubType.rulers_schema"
                :key="idx"
                class="px-3.5 py-1.5 bg-amber-50 text-amber-700 border border-amber-300 text-xs font-bold rounded-full flex items-center gap-1.5"
              >
                <span>👑</span>
                <span>{{ ruler }}</span>
              </span>
          ...
          <div class="bg-white/90 border border-slate-200 rounded-3xl p-6 shadow-xl space-y-4">
            <div class="flex items-center justify-between border-b border-slate-200 pb-3">
              <div>
                ...
                <p class="text-xs text-slate-500">Pre-configured officer roles for {{ clubType.name }}</p>
              </div>
              <span class="text-xs font-mono font-bold text-slate-700 px-2.5 py-1 bg-slate-100 rounded-lg border border-slate-300">
            ...
            <div class="space-y-2 max-h-[520px] overflow-y-auto pr-1">
              <div
                ...
                class="p-3 bg-slate-100 rounded-2xl border border-slate-200 flex items-center justify-between text-xs hover:border-slate-400 transition"
              >
                <div class="flex items-center gap-3">
                  <span class="w-6 h-6 flex items-center justify-center bg-slate-100 text-blue-700 font-mono font-black text-[11px] rounded-full border border-slate-300">
                    {{ role.rank_level || '—' }}
                  </span>
                  <div>
                    <span class="font-bold text-slate-900 block">{{ role.title }}</span>
                    <span class="text-[10px] text-blue-600 font-mono">Code: {{ role.short_code }}</span>
                  </div>
                </div>
                <span v-if="role.is_executive" class="px-2.5 py-0.5 bg-amber-50 text-amber-700 text-[9px] font-black uppercase rounded-md border border-amber-300">
                  Key Officer
                </span>
              </div>
      ...
      <div class="bg-white/90 border border-slate-200 rounded-3xl p-6 shadow-xl space-y-4">
        <div class="flex items-center justify-between border-b border-slate-200 pb-3">
          <div>
            ...
            <p class="text-xs text-slate-500">All active lodges/chapters of this specific Order registered on the platform.</p>
          </div>
          <span class="text-xs font-bold text-slate-500">Total {{ clubs.length }} club(s)</span>
        </div>

        <div v-if="clubs.length" class="border border-slate-200 rounded-2xl overflow-hidden shadow-2xs">
            ...
            <tbody class="divide-y divide-slate-200 text-slate-700">
              <tr v-for="c in clubs" :key="c.id" class="hover:bg-slate-50 transition-colors">
                ...
                <td class="py-3.5 px-4">
                  <span class="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-200">
                    🏛️ {{ c.province }}
                  </span>
                </td>
        ...
        <div v-else class="p-8 text-center bg-slate-50 rounded-2xl border border-slate-200 space-y-2">
          <span class="text-2xl block">🏛️</span>
          <h4 class="text-sm font-bold text-slate-700">No {{ clubType.name }} Clubs Registered Yet</h4>
