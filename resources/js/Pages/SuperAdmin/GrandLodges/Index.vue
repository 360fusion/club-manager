    ...
    <SuperAdminLayout title="Masonic Grand Lodges & Country Heads">
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-5">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Grand Lodges &amp; Country Governing Bodies</h1>
                    <p class="text-sm text-slate-500 mt-1">Manage national Masonic sovereign Grand Lodges (UGLE, Grand Lodge of Scotland, Ireland, Malta, etc.) and their regional jurisdictions.</p>
                </div>
                <button
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-white border border-slate-200 p-5 rounded-xl shadow-lg space-y-1">
                    <span class="text-[10px] font-bold text-blue-600 uppercase tracking-wider block">Governing Grand Lodges</span>
                    <span class="text-3xl font-black text-slate-900 block">{{ grandLodges.length }}</span>
                    <span class="text-xs text-slate-500 block font-medium">Sovereign Masonic Heads</span>
                </div>

                <div class="bg-white border border-slate-200 p-5 rounded-xl shadow-lg space-y-1">
                    <span class="text-[10px] font-bold text-blue-600 uppercase tracking-wider block">Total Managed Provinces</span>
                    <span class="text-3xl font-black text-slate-900 block">
                        {{ grandLodges.reduce((acc, gl) => acc + (gl.provinces_count || 0), 0) }}
                    </span>
                    <span class="text-xs text-slate-500 block font-medium">Provinces across all Grand Lodges</span>
                </div>

                <div class="bg-white border border-slate-200 p-5 rounded-xl shadow-lg space-y-1">
                    <span class="text-[10px] font-bold text-emerald-700 uppercase tracking-wider block">Official Web Portals</span>
                    <span class="text-3xl font-black text-slate-900 block">
                        {{ grandLodges.filter(gl => gl.website_url).length }}
                    </span>
                    <span class="text-xs text-slate-500 block font-medium">Active Grand Lodge websites</span>
                </div>

                <div class="bg-white border border-slate-200 p-5 rounded-xl shadow-lg space-y-1">
                    <span class="text-[10px] font-bold text-amber-700 uppercase tracking-wider block">Jurisdictional Countries</span>
                    <span class="text-3xl font-black text-slate-900 block">
                        {{ new Set(grandLodges.map(gl => gl.country)).size }}
                    </span>
                    <span class="text-xs text-slate-500 block font-medium">Global Sovereign Territories</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div
                    ...
                    :key="gl.id"
                    class="bg-white border border-slate-200 rounded-xl p-6 shadow-xl flex flex-col justify-between space-y-4 hover:border-slate-400 transition"
                >
                    <div class="space-y-3">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-center gap-3">
                                <span class="text-2xl p-2 bg-slate-100 border border-slate-200 rounded-lg shadow-inner">
                                    {{ getFlag(gl.country) }}
                                </span>
                                <div>
                                    <h2 class="text-lg font-bold text-slate-900 leading-tight">{{ gl.name }}</h2>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="px-2 py-0.5 rounded text-[11px] font-extrabold bg-blue-50 text-blue-700 border border-blue-200 font-mono">
                                            {{ gl.short_name || gl.code }}
                                        </span>
                                        <span class="text-xs text-slate-500 font-medium">
                                            Country: <strong class="text-slate-800">{{ gl.country }}</strong>
                                        </span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="pt-4 border-t border-slate-200 flex items-center justify-between text-xs">
                        <div class="flex items-center gap-4">
                            <div class="text-slate-700">
                                <span class="font-bold text-slate-900">{{ gl.provinces_count }}</span>
                                <span class="text-slate-500 ml-1">Provinces</span>
                            </div>
                            <div class="text-slate-700">
                                <span class="font-bold text-slate-900">{{ gl.clubs_count }}</span>
                        ...
                        <div class="flex items-center gap-2">
                            <a
                                ...
                                :href="gl.website_url"
                                target="_blank"
                                class="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 text-xs font-semibold rounded-lg transition inline-flex items-center gap-1.5"
                            >
                            ...
                            <button
                                @click="startEdit(gl)"
                                class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg border border-slate-300 font-semibold transition"
                            >
                                Edit
                            </button>
                            <button
                                @click="deleteGrandLodge(gl)"
                                class="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-lg border border-rose-200 font-semibold transition"
                            >
                                Delete
                            </button>
                        </div>
        ...
        <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            <div class="bg-white border border-slate-200 rounded-xl p-6 w-full max-w-lg shadow-2xl space-y-4">
                <div class="flex items-center justify-between border-b border-slate-200 pb-3">
                    <h3 class="text-lg font-bold text-slate-900">Add Masonic Grand Lodge</h3>
                ...
                <form @submit.prevent="submitCreate" class="space-y-4 text-xs">
                    <div>
                        <label class="block font-medium text-slate-700 mb-1">Grand Lodge Name</label>
                        <input
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block font-medium text-slate-700 mb-1">Abbreviation / Short Name</label>
                            <input
                                v-model="createForm.short_name"
                                type="text"
                                placeholder="e.g. GLoS"
                                class="w-full bg-slate-100 border border-slate-300 text-slate-900 rounded-md px-3 py-2 font-mono"
                            />
                        </div>
                        <div>
                    ...
                    <div>
                        <label class="block font-medium text-slate-700 mb-1">Sovereign Country / Territory</label>
                        <input
        ...
        <!-- Edit Modal -->
        <div v-if="editingGrandLodge" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            <div class="bg-white border border-slate-200 rounded-xl p-6 w-full max-w-lg shadow-2xl space-y-4">
                <div class="flex items-center justify-between border-b border-slate-200 pb-3">
                    <h3 class="text-lg font-bold text-slate-900">Edit {{ editingGrandLodge.name }}</h3>
                    <button @click="editingGrandLodge = null" class="text-slate-500 hover:text-slate-900">✕</button>
                </div>
                ...
                <form @submit.prevent="submitUpdate" class="space-y-4 text-xs">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            <label class="block font-medium text-slate-700 mb-1">Sovereign Country</label>
                            <input
