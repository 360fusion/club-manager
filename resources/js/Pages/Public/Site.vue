...
const themeClasses = computed(() => {
    ...
    if (key === 'light_navy') {
        return {
            wrapper: 'bg-white text-slate-900 font-sans selection:bg-sky-500 selection:text-white',
            header: 'sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200/90 shadow-sm',
            navActive: 'bg-slate-900 text-white font-bold shadow-md shadow-slate-900/10',
            navInactive: 'text-slate-600 hover:text-slate-900 border-transparent',
            heroBg: 'bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 text-white border border-slate-800 shadow-xl',
            heroPill: 'bg-sky-500/20 border-sky-400/30 text-sky-300 font-semibold',
            heroCta: 'bg-sky-500 hover:bg-sky-600 text-white font-bold shadow-sky-500/25',
            cardBg: 'bg-white border border-slate-200 text-slate-900 shadow-sm hover:shadow-md transition-shadow',
            accentText: 'text-sky-600',
            accentBg: 'bg-sky-600',
            footer: 'bg-slate-900 border-t border-slate-800 text-slate-400',
        };
    }
    if (key === 'executive_light') {
        return {
            wrapper: 'bg-slate-50 text-slate-900 font-sans selection:bg-indigo-600 selection:text-white',
            header: 'sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200/80 shadow-sm',
            navActive: 'bg-indigo-50 text-indigo-700 border-indigo-200 font-bold shadow-sm',
            navInactive: 'text-slate-600 hover:text-slate-900 border-transparent',
            heroBg: 'bg-white border border-slate-200/90 shadow-md text-slate-900',
            heroPill: 'bg-indigo-50 border border-indigo-200 text-indigo-700 font-semibold',
            heroCta: 'bg-gradient-to-r from-indigo-600 to-sky-600 text-white font-bold shadow-indigo-600/20',
            cardBg: 'bg-white border border-slate-200 text-slate-900 shadow-sm hover:border-indigo-200 transition-colors',
    ...
    if (key === 'masonic_light') {
        return {
            wrapper: 'bg-white text-slate-900 font-serif selection:bg-amber-500 selection:text-slate-950',
            header: 'sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b-2 border-amber-500/40 shadow-sm',
            navActive: 'bg-amber-50 text-amber-900 border-amber-300 font-sans font-bold shadow-sm',
            navInactive: 'text-slate-700 hover:text-amber-800 font-sans border-transparent',
            heroBg: 'bg-gradient-to-br from-blue-950 via-indigo-950 to-slate-950 text-white border-2 border-amber-500/40 shadow-xl',
            heroPill: 'bg-amber-500/20 border-amber-400/40 text-amber-300 font-sans tracking-widest',
            heroCta: 'bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 font-sans font-black shadow-amber-500/30',
            cardBg: 'bg-white border border-amber-500/30 text-slate-900 shadow-sm hover:border-amber-500/50 transition-colors',
            accentText: 'text-amber-700',
            accentBg: 'bg-amber-600',
            footer: 'bg-slate-900 border-t-2 border-amber-500/40 text-slate-400 font-sans',
        };
    }
    if (key === 'warm_light') {
        return {
            wrapper: 'bg-amber-50/30 text-slate-900 font-sans selection:bg-amber-500 selection:text-white',
            header: 'sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-amber-200/60 shadow-sm',
            navActive: 'bg-amber-100/70 text-amber-900 border-amber-300 font-bold',
            navInactive: 'text-slate-600 hover:text-amber-900 border-transparent',
            heroBg: 'bg-white border border-amber-200/80 shadow-sm text-slate-900',
            heroPill: 'bg-amber-100/60 border border-amber-300/60 text-amber-800 font-semibold',
            heroCta: 'bg-gradient-to-r from-amber-600 to-orange-600 text-white font-bold shadow-amber-600/20',
            cardBg: 'bg-white border border-amber-200/80 text-slate-900 shadow-sm hover:border-amber-300 transition-colors',
            accentText: 'text-amber-700',
            accentBg: 'bg-amber-600',
            footer: 'bg-white border-t border-amber-200/60 text-slate-600',
        };
    }
    if (key === 'obsidian') {
        return {
            wrapper: 'bg-slate-950 text-slate-100 font-sans selection:bg-sky-500 selection:text-white',
            header: 'sticky top-0 z-50 backdrop-blur-xl bg-slate-950/90 border-b border-slate-800',
            navActive: 'bg-sky-500/20 text-sky-400 border-sky-500/40 shadow-sm shadow-sky-500/10',
            ...
            heroPill: 'bg-sky-500/10 border-sky-500/30 text-sky-400',
            heroCta: 'bg-gradient-to-r from-sky-500 to-indigo-600 text-white shadow-sky-500/30',
    ...
    if (key === 'masonic') {
        return {
            ...
            heroBg: 'bg-gradient-to-br from-blue-950 via-slate-950 to-indigo-950 border-2 border-amber-500/30 shadow-2xl',
            heroPill: 'bg-amber-500/10 border-amber-500/30 text-amber-400 font-sans tracking-widest',
            heroCta: 'bg-gradient-to-r from-amber-600 to-amber-500 text-slate-950 font-black shadow-amber-500/20',
    ...
    if (key === 'minimal') {
        return {
            wrapper: 'bg-slate-50 text-slate-900 font-sans selection:bg-emerald-500 selection:text-white',
            header: 'sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-sm',
            navActive: 'bg-emerald-50 text-emerald-700 border-emerald-300 font-bold',
            navInactive: 'text-slate-600 hover:text-slate-900 border-transparent',
            heroBg: 'bg-white border border-slate-200 shadow-sm text-slate-900',
            heroPill: 'bg-emerald-50 border-emerald-200 text-emerald-700',
    ...
    if (key === 'vibrant') {
        return {
            wrapper: 'bg-slate-900 text-slate-100 font-sans selection:bg-rose-500 selection:text-white',
            ...
            heroBg: 'bg-gradient-to-r from-rose-900 via-amber-900 to-purple-950 border border-rose-500/30 shadow-2xl',
    ...
    return {
        ...
        navActive: 'bg-sky-500/10 text-sky-400 border-sky-500/30',
        navInactive: 'text-slate-400 hover:text-white border-transparent',
        heroBg: 'bg-gradient-to-r from-slate-900 via-sky-950/40 to-slate-900 border border-slate-800',
        heroPill: 'bg-sky-500/10 border-sky-500/20 text-sky-400',
        heroCta: 'bg-gradient-to-r from-sky-500 to-indigo-600 text-white shadow-sky-500/20',
        cardBg: 'bg-slate-900 border border-slate-800 text-slate-100 shadow-xl',
        accentText: 'text-sky-400',
        accentBg: 'bg-sky-500',
        footer: 'bg-slate-950 border-t border-slate-800 text-slate-400',
    };
...
<template>
    ...
    <div :class="['min-h-screen transition-colors duration-300', themeClasses.wrapper]">
        ...
        <div v-if="previewTheme" class="bg-amber-400 text-amber-950 px-4 py-2.5 text-center text-xs font-bold sticky top-0 z-[100] flex items-center justify-center gap-3 shadow-md border-b border-amber-500/30">
            ...
            <Link :href="`/clubs/${club.slug}/admin/pages`" class="ml-2 px-2.5 py-1 rounded bg-amber-950 text-white font-bold hover:bg-black transition-colors text-[11px]">
            ...
            <div v-for="(block, index) in page.blocks" :key="index" class="max-w-7xl mx-auto px-6">
                ...
                <section v-if="block.type === 'hero'" class="relative p-10 sm:p-16 rounded-3xl bg-gradient-to-r from-slate-900 via-sky-950/40 to-slate-900 border border-slate-800 text-center overflow-hidden">
                    <div class="relative z-10 max-w-3xl mx-auto space-y-6">
                        ...
                        <div v-if="block.cta_text && block.cta_link" class="pt-2">
                            <a :href="getSiteUrl(block.cta_link)" class="inline-block py-3.5 px-7 rounded-2xl bg-gradient-to-r from-sky-500 to-indigo-600 text-white font-bold text-sm shadow-xl shadow-sky-500/20 hover:scale-105 transition-transform">
                                {{ block.cta_text }}
                            </a>
                        </div>
                    </div>
                ...
                <section v-else-if="block.type === 'button' && block.url" class="max-w-4xl mx-auto">
                    <div :class="['flex', block.align === 'left' ? 'justify-start' : block.align === 'right' ? 'justify-end' : 'justify-center']">
                        <a :href="getSiteUrl(block.url)" class="py-3 px-6 rounded-xl bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-400 hover:to-indigo-500 text-white font-bold text-sm shadow-lg shadow-sky-500/20 transition-all hover:scale-105">
                ...
                <section v-else-if="block.type === 'pricing_cards'" class="max-w-6xl mx-auto space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div v-for="plan in membershipPlans" :key="plan.id" class="p-6 rounded-3xl bg-slate-900 border border-slate-800 space-y-4 flex flex-col justify-between">
                            <div>
                                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 uppercase tracking-wider">
                                    {{ plan.billing_period }}
                                </span>
                            ...
                            <div class="pt-4 border-t border-slate-800 flex items-baseline justify-between">
                                ...
                                <Link :href="`/clubs/${club.slug}/admin/subscriptions`" class="py-2 px-4 rounded-xl bg-indigo-600 text-white font-bold text-xs hover:bg-indigo-500">
                                    Subscribe →
                                </Link>
                            </div>
                        </div>
                ...
                <section v-else-if="block.type === 'news_feed' || block.type === 'news_list'" class="space-y-6">
                    ...
                    <div v-if="getNewsTotalPages(block) > 1" class="flex items-center justify-between pt-6 border-t border-slate-800">
                        <button
                            ...
                            class="py-2.5 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 disabled:opacity-40 text-xs font-bold text-slate-200 border border-slate-800 flex items-center gap-1.5 cursor-pointer disabled:cursor-not-allowed transition-all"
                        >
                            ← Previous Page
                        </button>
                ...
                <section v-else-if="block.type === 'contact_details'" class="py-8 space-y-10 text-center">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
                        <!-- Email Card -->
                        <div class="p-8 rounded-2xl bg-amber-50/10 border border-amber-500/20 text-center space-y-4 hover:border-amber-500/40 transition-all">
                            <div class="w-14 h-14 rounded-full bg-slate-900 border border-amber-500/30 text-amber-400 flex items-center justify-center mx-auto text-xl shadow-lg">
                                ✉️
                            </div>
