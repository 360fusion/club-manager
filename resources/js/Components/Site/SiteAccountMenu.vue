...
const item = 'block w-full text-left rounded-xl px-3 py-2 text-xs font-semibold text-neutral-700 hover:bg-neutral-100 dark:text-neutral-200 dark:hover:bg-white/10';
</script>
...
<template>
    <div class="relative shrink-0" @keydown.esc="open = false">
        ...
        <template v-else>
            ...
            <div v-if="open" role="menu" class="absolute right-0 z-50 mt-2 w-56 space-y-1 rounded-2xl border border-neutral-200 bg-white p-2 text-neutral-800 shadow-xl dark:border-neutral-700 dark:bg-[var(--cm-night-soft)] dark:text-neutral-100">
                <div class="rounded-xl bg-neutral-50 p-3 dark:bg-white/10">
                    <div class="truncate text-xs font-bold text-neutral-900 dark:text-white">{{ user.name }}</div>
                    <div class="truncate text-[11px] text-neutral-500 dark:text-neutral-400">{{ user.email }}</div>
                </div>

                <Link v-for="link in links" :key="link.href" :href="link.href" role="menuitem" :class="item" @click="open = false">{{ link.label }}</Link>

                <Link v-if="isStaff" :href="`/${club.slug}/overview`" role="menuitem" :class="[item, 'border-t border-neutral-100 dark:border-neutral-800 rounded-t-none mt-1 pt-2']" @click="open = false">Club admin</Link>

                <div class="border-t border-neutral-100 pt-1 dark:border-neutral-800">
